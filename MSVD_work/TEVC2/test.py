...
from loop.run import training, validation, test, predict_TBoard
...
def main(cfg):
    ...
    if cfg.use_flow:
        model = TEVCTransformer(
            test_ds.voc_size, cfg.d_model, cfg.H, cfg.N,
    ...
    model.load_state_dict(torch.load('./results/model_param/msrvtt/training/train_6.pth'))
    # all_vocab = torch.load('./vocab.pt')
    creterion = LabelSmoothing(cfg.smoothing, test_ds.pad_idx)
    ...
    for i, batch in enumerate(tqdm(test_loader, desc=f'test')):
        if count == 10:
            break
        _, _, _, video_names = batch
        print(video_names)
        # with torch.no_grad():
        text, texts = predict_TBoard(video_names, test_loader, model)
        print(text)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--use_flow', type=bool, default=True
    )

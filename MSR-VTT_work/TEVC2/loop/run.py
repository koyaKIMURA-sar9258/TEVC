...
def training(model, loader, loss_compute, lr_scheduler, epoch, TBoard, dataset_name, use_flow):
    ...
    for i, batch in enumerate(tqdm(loader, desc=f'{time} train ({epoch})', disable=True)):
        # if count == 10:
        #     break
...
def validation(model, loader, loss_compute, lr_scheduler, epoch,
    ...
    for i, batch in enumerate(tqdm(loader, desc=f'{time} eval ({epoch})', disable=True)):
        # if count == 10:
        #     break
...
def test(model, loader, dataset_name, val_avemeteor_epoch, TBoard, vocab, use_flow, best_epoch):
    ...
    best_model_path = f'./results/model_param/{dataset_name}/training/train_37.pth'
    time = strftime('%X', localtime())
    ...
    for i, batch in enumerate(tqdm(loader, desc=f'{time} test ', disable=True)):
        # if count == 30:
        #     break
    ...
    max_metrics = (all_max_meteor, all_max_bleu3, all_max_bleu4)
